%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Description: 
%           GSA_CA fuses the upsampled MultiSpectral (MS) and PANchromatic (PAN) images by 
%           exploiting the Gram-Schmidt Adaptive_Context adaptive(GSA_CA) algorithm and lcoal basedinjection model.
% 
% Interface:
%           I_Fus_GSA_CA = GSA_CA(I_MS,I_PAN,I_MS_LR,ratio)
%
% Inputs:
...
% 在GSA函数的基础上修改，将全局的注入模型更改为局部只适应窗口
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function I_Fus_GSA = GSA_CA(I_MS,I_PAN,I_MS_LR,ratio)
...
for i = 1:dim  
    ...
    gm(:,:,i+1) = calLcoalCova(I0,I_MS(:,:,i),r)./(calLcoalCova(I0,I0,r));
