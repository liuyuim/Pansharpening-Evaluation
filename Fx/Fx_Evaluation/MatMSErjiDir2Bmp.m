function [] = MatMSErjiDir2Bmp (FusionImgYijiPath,Step,saveDir)
addpath(genpath('.\Toolbox\'));
    ...
    for i_ErjiDir = 3 : ErjiDir_list_Nums
        ...
        for i = 1:NumImgs
            % 判断是否每若干张一处理的Step
            if mod(i, Step) == 0
                formatSpec = '正在处理目录 %s！%d个图像中每 %d 一处理，这是第%d个！\n';
    ...
    %             SaveName = fullfile(saveDir,[num2str(i),'.bmp']);
                ...
                [rgb] = func_hyperImshow(MS,[1,2,3]); % func_hyperImshow(3D高光谱图像,[R,G,B])
