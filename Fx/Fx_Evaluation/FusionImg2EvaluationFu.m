...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [ ] = FusionImg2EvaluationFu (FusionImgYijiPath,TestYijiPath,saveDir)
    ...
    for i_ErjiDir = 3 : ErjiDir_list_Nums
        ...
        for i_NumImgs = 1:NumImgs
            ...
            [D_lambda,D_S,QNR,SAM_MS,SCC_Pan] = indexes_evaluation_FS(I_F,I_MS_LR,I_PAN,L,thvalues,I_MS,sensor,im_tag,ratio);
            ...
            ] = FusionImg2EvaluationMetricFu(I_F,I_MS_Up); % %cross entropy交叉熵暂时借用一下I_MS_Up参与运算
                    
            
            %% 写入表中
            MatrixResult_Fu(1,:) = [D_lambda,D_S,QNR,SAM_MS,SCC_Pan,...  
                ...     Q_index,SAM_index,ERGAS_index,sCC,Q2n_index,...
                ...RB, RV, RSD, RMSE_, QAVE_, CCMean,
                SD,entropy_,CEMean,SFMean];
...
%             saveName = fullfile(saveDir,ErjiDir_list(ErjiDir_i).name,[num2str(i),'.mat']);
            ...
            XlsxTitle = [ "D_lambda", "D_S", "QNR", "SAM_MS", "SCC_Pan",  ... 
                ...   "Q_index", "SAM_GT", "ERGAS_index", "SCC_GT", "Q2n_index", ...
                ... "RB", "RV", "RSD", "RMSE_", "QAVE_", "CCMean", 
                "SD", "entropy_", "CEMean", "SFMean", "Path->"]; % 指标标题
...
%             writematrix(XlsxValue,saveXlsxName,'WriteMode','append')
        ...
        %计算方差
        Var = var(MatrixResults_Fu,0,3 ); % MeanVar = [Mean_Fu,'±',Var] ;
        writematrix('方差',saveXlsxName,'WriteMode','append')
        writematrix(Var,saveXlsxName,'WriteMode','append')  
        %计算中值 
