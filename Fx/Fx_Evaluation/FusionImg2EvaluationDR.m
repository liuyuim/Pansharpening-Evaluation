...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [ ] = FusionImg2EvaluationDR (FusionImgYijiPath,TestYijiPath,saveDir)
    ...
    for i_ErjiDir = 3 : ErjiDir_list_Nums
        ...
        for i_NumImgs = 1:NumImgs
            ...
            %% indexes_evaluation
            I_GT = double(TestData.gt); %ground truth 这里。

            Qblocks_size = 32;
            flag_cut_bounds = 0;%不进行裁切
            dim_cut = 0;%裁剪的大小不设置

            [Q_index, SAM_GT, ERGAS_index, SCC_GT, Q2n_index] = indexes_evaluation(I_F,I_GT,ratio,L,Qblocks_size,flag_cut_bounds,dim_cut,thvalues);
...
%             saveName = fullfile(saveDir,ErjiDir_list(ErjiDir_i).name,[num2str(i),'.mat']);
            ...
            XlsxTitle = [ ..."D_lambda", "D_S", "QNR", "SAM_MS", "SCC_Pan", ...                
                "Q_index", "SAM_GT", "ERGAS_index", "SCC_GT", "Q2n_index", ...
                "RB", "RV", "RSD", "RMSE_", "QAVE_", "CCMean", "Path->"]; % 指标标题
                 ..."SD", "entropy_", "CEMean", "SFMean", 
